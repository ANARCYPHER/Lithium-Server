...
uint256 hashGenesisBlock("0x8711d721c81f8263a69a58bbd46e8df0e621c908ac7b80a9786687dfc5e508f5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lithium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lithium Signed Message:\n";
...
{
    // Lithium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Lithium: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Lithium: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Lithium: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lithium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lithium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3e1daa12e60f2f3c4efd8b9fef3533e4489760a885be20f0ef84ebe5713c6283");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Lithium stopping our Anxiety";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0423e596be002a8cebc86be01e54a7845d2b674035f6e400496363905d018398f4ca7dc8f2b179569884a6859f27964c0c87e77137ee8515506faf8098e5d39e0b") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1614516491;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085044452;

        if (fTestNet)
        {
            block.nTime    = 1614516404;
            block.nNonce   = 390342755;
        }
        ////Paste Here!!
        ...
        assert(block.hashMerkleRoot == uint256("0xe508a729a0067c5df3c689addeb49da0158d3dd1d2a90291d2d44be8ea3d7472"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Lithium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LithiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LithiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LithiumMiner(CWallet *pwallet)
{
    printf("LithiumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lithium-miner");
    ...
    try { loop {
        ...
        printf("Running LithiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LithiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LithiumMiner, pwallet));
}
